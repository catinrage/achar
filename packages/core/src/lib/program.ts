...
import { createLogger, LogLevel, measureAsyncPerformance } from './logger';
 ...
 * @description Result of program execution
 */
export interface ProgramExecutionResult {
  ...
  /**
   * Collected errors during execution
   */
...
export class Program {
   ...
   * @description Enhanced execute method with comprehensive error handling and monitoring
   * @param {ProgramExecutionOptions} options - Execution options
   * @returns {Promise<ProgramExecutionResult>} Execution result with statistics
   */
  public async executeWithOptions(
    ...
    return measureAsyncPerformance(
      'executeWithOptions',
      async () => {
        const result: ProgramExecutionResult = {
          success: false,
          eventsProcessed: 0,
          eventsSkipped: 0,
          errors: [],
        ...
        try {
          ...
          for (const event of this._events) {
            ...
            try {
              ...
              if (mergedOptions.logProgress && eventIndex % 10 === 0) {
                this._logger.debug(
                  `Processing event ${eventIndex}/${this._events.length}`,
                  {
                    progress:
                      ((eventIndex / this._events.length) * 100).toFixed(1) +
                      '%',
                    eventName: event.name,
                  },
                  'executeWithOptions',
              ...
              if (mergedOptions.validateEvents) {
              ...
              const metadata: EventListenerMetadata = {
                ...
                currentFile: () => {
                  if (!this._builder) {
                    throw new EventProcessingError(
                      'Builder not initialized',
                      createErrorContext('Program', 'executeWithOptions', {
                        eventName: event.name,
                      }),
                    );
                  }
                ...
                next:
                  eventIndex < this._events.length - 1
                    ? this._events[eventIndex + 1]
                    : null,
                previous: eventIndex > 0 ? this._events[eventIndex - 1] : null,
                findLastEvent: <T extends keyof EventsType>(eventName: T) => {
                ...
                findNearestEvent: <T extends keyof EventsType>(
                  ...
                  for (let i = eventIndex + 1; i < this._events.length; i++) {
                    if (this._events[i].name === eventName) {
                      return this._events[i] as Event<T>;
                    }
                  }
                ...
                findLastEventOrThrow: <T extends keyof EventsType>(
                  eventName: T,
                ) => {
                  const result = metadata.findLastEvent(eventName);
                  if (!result) {
                    throw new EventProcessingError(
                      `Could not find last event '${eventName}'`,
                      createErrorContext('Program', 'executeWithOptions', {
                ...
                findNearestEventOrThrow: <T extends keyof EventsType>(
                  ...
                  const result = metadata.findNearestEvent(eventName);
                  if (!result) {
                    throw new EventProcessingError(
                      `Could not find nearest event '${eventName}'`,
                      createErrorContext('Program', 'executeWithOptions', {
                ...
                findNthNextEventOrThrow: <T extends keyof EventsType>(
                  ...
                  const result = metadata.findNthNextEvent(eventName, n);
                  if (!result) {
                    throw new EventProcessingError(
                      `Could not find ${n}th next event '${eventName}'`,
                      createErrorContext('Program', 'executeWithOptions', {
                ...
                findNthPreviousEventOrThrow: <T extends keyof EventsType>(
                  ...
                  const result = metadata.findNthPreviousEvent(eventName, n);
                  if (!result) {
                    throw new EventProcessingError(
                      `Could not find ${n}th previous event '${eventName}'`,
                      createErrorContext('Program', 'executeWithOptions', {
              ...
              result.statistics.successfulEvents++;

              this._logger.debug(
                `Event ${event.name} processed successfully`,
                {
                  eventName: event.name,
                  eventIndex,
              ...
              const processingError = new EventProcessingError(
                `Failed to process event ${event.name} at index ${eventIndex}: ${
                  error instanceof Error ? error.message : String(error)
                }`,
                createErrorContext(
                  'Program',
              ...
              result.statistics.failedEvents++;

              this._logger.logError(processingError, 'executeWithOptions');
              ...
              if (result.errors.length >= (mergedOptions.maxErrors || 50)) {
                this._logger.error(
                  `Maximum error count reached (${mergedOptions.maxErrors}), stopping execution`,
                  undefined,
                  {
          ...
          if (this._builder) {
            this._builder.flush();
            const builtFiles = this._builder.build();
            builtFiles.forEach((file) => {
              // Store the file name and its code content
              result.files.set(file.file, file);
            });
          }

          const endTime = performance.now();
          result.statistics.executionTime = endTime - startTime;
          result.statistics.averageEventTime =
            result.statistics.executionTime / result.statistics.totalEvents;
          result.success = result.statistics.failedEvents === 0;
          ...
          this._logger.logError(wrappedError, 'executeWithOptions');
          throw wrappedError;
        }
      },
      this._logger,
  ...
  /**
   * @private
   * @method _validateEvent
   * @description Validates an event before processing
   */
  private _validateEvent(event: Event<keyof EventsType>): void {
    try {
      // Validate event name
      InputValidators.validateEventName(event.name as string, 'Program');

      // Validate event data
      InputValidators.validateEventParameters(
