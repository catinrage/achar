...
describe('Program', () => {
  ...
  describe('listener failures', () => {
    /**
     * Listeners used to run inside a `Promise.race` against a 5-second timer.
     * The race could never be won by the timer — the listener ran synchronously
     * in the promise executor, so it had already settled — but it did make a
     * throwing listener reject asynchronously, which meant the `catch` in
     * `trigger` never ran: nothing was collected, `continueOnError` never
     * applied, and the error escaped the process as an unhandled rejection.
     * These pin the behaviour the wrapper was hiding.
     */
    it('collects a listener error instead of letting it escape', () => {
      // Arrange
      const failing = new Program({ programName: 'P' });
      failing.on('StartOfFile', () => {
      ...
      // Assert
      expect(failing.getErrors()).toHaveLength(1);
      expect(failing.getErrors()[0].message).toContain('listener blew up');
    });

    it('keeps processing later events when continueOnError is set', () => {
      // Arrange
      const failing = new Program({ programName: 'P' });
      failing.on('StartOfFile', ($) => {
        $.Comment('before');
      ...
      // Assert
      expect(failing.generate()[0].code).toContain('after');
    });

    it('rethrows when continueOnError is disabled', () => {
      // Arrange
      const failing = new Program({ programName: 'P' });
      failing.setExecutionOptions({ continueOnError: false });
      failing.on('StartOfFile', () => {
        throw new Error('boom');
      });
      failing.loadEvents([
      ...
      // Act + Assert
      expect(() => failing.process()).toThrow(/boom/);
    });

    /**
     * Each listener call used to leave an uncleared 5-second `setTimeout`
     * behind, so a real trace pinned hundreds of thousands of timers open and
     * the process could not exit until the last one fired — about five seconds
     * added to every run. Nothing may schedule work that outlives `process()`.
     */
    it('schedules no timers that outlive processing', async () => {
      // Arrange
      ...
      const timed = new Program({ programName: 'P' });
      timed.on('StartOfFile', ($) => {
        $.Comment('work');
      });
      timed.loadEvents([
        { _eventName: 'StartOfFile', _index: 0 },
